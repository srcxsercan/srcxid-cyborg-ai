...
echo "📡 CYBORG-OS v17: Starship Telemetry Core oluşturuluyor..."
...
# === TELEMETRY CONFIG ===
cat << 'CFG' > src/starship/telemetry-config.json
{
  "sinks": {
    "file": {
      "enabled": true,
...
# === TELEMETRY LOG CLI (EVENT INGEST) ===
cat << 'LOG' > src/cli/cyborg-telemetry-log.js
#!/usr/bin/env node
import fs from "fs";
...
# === TELEMETRY REPORT CLI (SUMMARY) ===
cat << 'REP' > src/cli/cyborg-telemetry-report.js
#!/usr/bin/env node
import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";
...
chmod +x src/cli/cyborg-telemetry-log.js
chmod +x src/cli/cyborg-telemetry-report.js

echo "✅ CYBORG-OS v17 (Telemetry Core + CLI'ler) tamamlandı!"
