...
echo "🤖 CYBORG-OS v16: Starship Autopilot oluşturuluyor..."

mkdir -p src/starship
mkdir -p src/cli
mkdir -p autopilot/output

# === AUTOPILOT CONFIG (hedefler) ===
cat << 'CFG' > src/starship/autopilot-targets.json
{
  "targets": ["docker-compose", "k8s"],
  "default_target": "docker-compose"
}
CFG

# ⚠️ v15 deployment.json YOKSA USER'A HATA VER
if [ ! -f "src/starship/deployment.json" ]; then
  echo "❌ src/starship/deployment.json bulunamadı."
  echo "   Önce v15 Starship Deployment Matrix'i kurmalısın."
  echo "   Örnek: ./cyborg-init-v15.sh"
fi

# === AUTOPILOT CLI ===
cat << 'CLI' > src/cli/cyborg-autopilot.js
#!/usr/bin/env node
import fs from "fs";
...
chmod +x src/cli/cyborg-autopilot.js

echo "✅ CYBORG-OS v16 (Starship Autopilot + cyborg-autopilot) tamamlandı!"
